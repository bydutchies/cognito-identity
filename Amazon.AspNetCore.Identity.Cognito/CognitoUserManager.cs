...
// ********** BEGIN MAATWERK **********
        ...
        /// <param name="authWorkflowSessionId">The ongoing Cognito authentication workflow id.</param>
        /// <returns>The <see cref="Task"/> that represents the asynchronous operation, containing the AuthFlowResponse object linked to that authentication workflow.</returns>
        public virtual Task<AuthFlowResponse> RespondToMfaSetupChallengeAsync(TUser user, string authWorkflowSessionId)
        {
            ThrowIfDisposed();
            ...
            return _userStore.RespondToMfaSetupChallengeAsync(user, ChallengeNameType.MFA_SETUP, authWorkflowSessionId, CancellationToken);
        }
// ********** EINDE MAATWERK **********
...
// ********** BEGIN MAATWERK **********
        public virtual Task<AuthFlowResponse> RespondToPasswordChangeChallengeAsync(TUser user, string newPassword, string authWorkflowSessionId)
        {
            ThrowIfDisposed();
            ...
            return _userStore.RespondToPasswordChangeChallengeAsync(user, newPassword, authWorkflowSessionId, CancellationToken);
        }
// ********** EINDE MAATWERK **********
...
// ********** BEGIN MAATWERK **********
        public async Task<AssociateSoftwareTokenResponse> AssociateSoftwareTokenAsync(string session, string accessToken)
        {
            ThrowIfDisposed();

            if (string.IsNullOrEmpty(session) && string.IsNullOrEmpty(accessToken))
                throw new ArgumentNullException(nameof(session));

            return await _userStore.AssociateSoftwareTokenAsync(session, accessToken, CancellationToken).ConfigureAwait(false);
        }

        public async Task<VerifySoftwareTokenResponse> VerifySoftwareTokenAsync(string session, string accessToken, string code)
        {
            ThrowIfDisposed();
            ...
            return await _userStore.VerifySoftwareTokenAsync(session, accessToken, code, CancellationToken).ConfigureAwait(false);
        }

        public async Task<SetUserMFAPreferenceResponse> SetUserMFAPreferenceAsync(string accessToken, bool enable)
        {
            ThrowIfDisposed();
            ...
            return await _userStore.SetUserMFAPreferenceAsync(accessToken, enable, CancellationToken).ConfigureAwait(false);
        }
// ********** EINDE MAATWERK **********
    }
}
