...
// ********** BEGIN MAATWERK **********
        private const string CognitoMFASetupWorkflowKey = "CognitoMFASetupWorkflowId";
        private const string CognitoMFASetupIdToken = "CognitoMFASetupIdToken";
        private const string CognitoMFASetupAccessToken = "CognitoMFASetupAccessToken";
...
// ********** BEGIN MAATWERK **********
            else if (checkPasswordResult.ChallengeName == ChallengeNameType.NEW_PASSWORD_REQUIRED)
            {
                signinResult = CognitoSignInResult.PasswordChangeRequired;

                var userPrincipal = new ClaimsPrincipal();
                userPrincipal.AddIdentity(new ClaimsIdentity(new List<Claim>() {
                    new Claim(ClaimTypes.Name, user.UserID),
                    new Claim(CognitoPasswordChangeWorkflowKey, checkPasswordResult.SessionID),
                }, IdentityConstants.ApplicationScheme));

                // This signs in the user in the context of mfa setup only. 
                await Context.SignInAsync(IdentityConstants.TwoFactorUserIdScheme, userPrincipal).ConfigureAwait(false);
            }
            else if (checkPasswordResult.ChallengeName == ChallengeNameType.MFA_SETUP)
            {
                signinResult = CognitoSignInResult.MfaSetupRequired;

                // Start associate Software token
                var associateResponse = await _userManager.AssociateSoftwareTokenAsync(checkPasswordResult.SessionID, null).ConfigureAwait(false);

                var userPrincipal = new ClaimsPrincipal();
                userPrincipal.AddIdentity(new ClaimsIdentity(new List<Claim>() {
                    new Claim(ClaimTypes.Name, user.UserID),
                    new Claim(CognitoMFASetupWorkflowKey, associateResponse.Session),
                    new Claim(CognitoMFASetupSecret, associateResponse.SecretCode),
                }, IdentityConstants.ApplicationScheme));

                // This signs in the user in the context of mfa setup only. 
                await Context.SignInAsync(IdentityConstants.TwoFactorUserIdScheme, userPrincipal).ConfigureAwait(false);
            }
            else if (string.IsNullOrEmpty(user.PreferredMfaSetting))
            {
                // User is logged in, but mfa is required
                signinResult = CognitoSignInResult.MfaSetupRequired;

                // Start associate Software token
                var associateResponse = await _userManager.AssociateSoftwareTokenAsync(null, user.SessionTokens.AccessToken).ConfigureAwait(false);

                var userPrincipal = new ClaimsPrincipal();
                userPrincipal.AddIdentity(new ClaimsIdentity(new List<Claim>() {
                    new Claim(ClaimTypes.Name, user.UserID),
                    new Claim(CognitoMFASetupIdToken, checkPasswordResult.AuthenticationResult.IdToken),
                    new Claim(CognitoMFASetupAccessToken, checkPasswordResult.AuthenticationResult.AccessToken),
                    new Claim(CognitoMFASetupRefreshToken, checkPasswordResult.AuthenticationResult.RefreshToken),
                    new Claim(CognitoMFASetupExpiresIn, checkPasswordResult.AuthenticationResult.ExpiresIn.ToString()),
                    new Claim(CognitoMFASetupSecret, associateResponse.SecretCode),
                }, IdentityConstants.ApplicationScheme));
...
// ********** BEGIN MAATWERK **********
        ...
        /// <returns>The task object representing the asynchronous operation containing the <see name="SignInResult"/>
        /// for the sign-in attempt.</returns>
        public async Task<SignInResult> RespondToMfaSetupChallengeAsync(string code, bool isPersistent)
        {
            var mfaSetupInfo = await RetrieveMfaSetupInfoAsync().ConfigureAwait(false);
            if (mfaSetupInfo == null || string.IsNullOrWhiteSpace(mfaSetupInfo.UserId))
                return SignInResult.Failed;

            var user = await _userManager.FindByIdAsync(mfaSetupInfo.UserId).ConfigureAwait(false);
            if (user == null)
                return SignInResult.Failed;

            var verifyResponse = await _userManager.VerifySoftwareTokenAsync(mfaSetupInfo.CognitoMFASetupWorkflowId, mfaSetupInfo.CognitoMFASetupAccessToken, code).ConfigureAwait(false);
            if (verifyResponse == null)
                return SignInResult.Failed;

            if (string.IsNullOrEmpty(mfaSetupInfo.CognitoMFASetupWorkflowId)) // Userpool MFA Optional
            { 
                // Set mfa preference
                await _userManager.SetUserMFAPreferenceAsync(mfaSetupInfo.CognitoMFASetupAccessToken, true).ConfigureAwait(false);

                // Set session tokens
                user.SessionTokens = GetCognitoUserSession(mfaSetupInfo);
            }
            else // Userpool MFA Required
            { 
                // Perform auth challenge
                await _userManager.RespondToMfaSetupChallengeAsync(user, verifyResponse.Session).ConfigureAwait(false);
        ...
        /// <summary>
        /// Gets the secret code for the current mfa setup, as an asynchronous operation.
        /// </summary>
        /// <returns>The task object representing the asynchronous operation containing the secret code
        /// for the mfa setup.</returns>
        public async Task<string> GetMfaSetupSecretCodeAsync()
        {
            var info = await RetrieveMfaSetupInfoAsync().ConfigureAwait(false);
            if (info == null)
                return null;

            return info.CognitoMFASetupSecretCode;
        }
        ...
        /// <returns>The task object representing the asynchronous operation containing the <typeparamref name="TUser"/>
        /// for the sign-in attempt.</returns>
        public async Task<TUser> GetMfaSetupUserAsync()
        {
            var info = await RetrieveMfaSetupInfoAsync().ConfigureAwait(false);
            if (info == null)
                return null;
        ...
        /// <summary>
        /// Retrieves the information related to the mfa setup workflow.
        /// </summary>
        /// <returns></returns>
        private async Task<MfaSetupInfo> RetrieveMfaSetupInfoAsync()
        {
            var result = await Context.AuthenticateAsync(IdentityConstants.TwoFactorUserIdScheme).ConfigureAwait(false);
            if (result?.Principal != null)
            {
                ...
                {
                    UserId = result.Principal.FindFirstValue(ClaimTypes.Name),
                    CognitoMFASetupWorkflowId = result.Principal.FindFirstValue(CognitoMFASetupWorkflowKey),
                    CognitoMFASetupIdToken = result.Principal.FindFirstValue(CognitoMFASetupIdToken),
                    CognitoMFASetupAccessToken = result.Principal.FindFirstValue(CognitoMFASetupAccessToken),
                    CognitoMFASetupRefreshToken = result.Principal.FindFirstValue(CognitoMFASetupRefreshToken),
                    CognitoMFASetupExpiresIn = result.Principal.FindFirstValue(CognitoMFASetupExpiresIn),
                    CognitoMFASetupSecretCode = result.Principal.FindFirstValue(CognitoMFASetupSecret)
                };
            }
            return null;
        }
        ...
        /// <summary>
        /// Utility class to model information related to the ongoing mfa setup workflow.
        /// </summary>
        internal class MfaSetupInfo
        {
            public string UserId { get; set; }
            public string CognitoMFASetupWorkflowId { get; set; }
            public string CognitoMFASetupIdToken { get; set; }
            public string CognitoMFASetupAccessToken { get; set; }
            public string CognitoMFASetupRefreshToken { get; set; }
            public string CognitoMFASetupExpiresIn { get; set; }
            public string CognitoMFASetupSecretCode { get; set; }
        }
        ...
        private CognitoUserSession GetCognitoUserSession(MfaSetupInfo info)
        {
            DateTime currentTime = DateTime.UtcNow;

            return new CognitoUserSession(
                info.CognitoMFASetupIdToken, 
                info.CognitoMFASetupAccessToken, 
                info.CognitoMFASetupRefreshToken, 
                currentTime, 
                currentTime.AddSeconds(Convert.ToDouble(info.CognitoMFASetupExpiresIn)));
        }
        ...
        /// <returns>The task object representing the asynchronous operation containing the <see name="SignInResult"/>
        /// for the sign-in attempt.</returns>
        public async Task<SignInResult> RespondToPasswordChangeChallengeAsync(string newPassword)
        {
            var passwordChangeInfo = await RetrievePasswordChangeInfoAsync().ConfigureAwait(false);
            if (passwordChangeInfo == null || string.IsNullOrWhiteSpace(passwordChangeInfo.UserId))
                return SignInResult.Failed;

            var user = await _userManager.FindByIdAsync(passwordChangeInfo.UserId).ConfigureAwait(false);
            if (user == null)
                return SignInResult.Failed;

            // Responding to the Cognito challenge.
            var challengeResult = await _userManager.RespondToPasswordChangeChallengeAsync(user, newPassword, passwordChangeInfo.CognitoPasswordChangeWorkflowId).ConfigureAwait(false);

            SignInResult signinResult;

            if (challengeResult == null)
                signinResult = SignInResult.Failed;
            else if (challengeResult.ChallengeName == ChallengeNameType.SMS_MFA || challengeResult.ChallengeName == ChallengeNameType.SOFTWARE_TOKEN_MFA)
            {
                signinResult = SignInResult.TwoFactorRequired;
                ...
                userPrincipal.AddIdentity(new ClaimsIdentity(new List<Claim>() {
                    new Claim(ClaimTypes.Name, user.UserID),
                    new Claim(Cognito2FAAuthWorkflowKey, challengeResult.SessionID),
                    new Claim(ClaimTypes.AuthenticationMethod, Cognito2FAProviderKey),
                    new Claim(Cognito2FAChallengeNameType, challengeResult.ChallengeName),
                }, IdentityConstants.ApplicationScheme));

                // This signs in the user in the context of 2FA only. 
                await Context.SignInAsync(IdentityConstants.TwoFactorUserIdScheme, userPrincipal).ConfigureAwait(false);
            }
            else if (challengeResult.ChallengeName == ChallengeNameType.MFA_SETUP)
            {
                signinResult = CognitoSignInResult.MfaSetupRequired;

                // Start associate Software token
                var associateResponse = await _userManager.AssociateSoftwareTokenAsync(challengeResult.SessionID, null).ConfigureAwait(false);
            ...
            else if (string.IsNullOrEmpty(challengeResult.SessionID))
            {
                // User is logged in, but mfa is required
                signinResult = CognitoSignInResult.MfaSetupRequired;

                // Start associate Software token
                var associateResponse = await _userManager.AssociateSoftwareTokenAsync(null, challengeResult.AuthenticationResult.AccessToken).ConfigureAwait(false);

                var userPrincipal = new ClaimsPrincipal();
                userPrincipal.AddIdentity(new ClaimsIdentity(new List<Claim>() {
                    new Claim(ClaimTypes.Name, user.UserID),
                    new Claim(CognitoMFASetupIdToken, challengeResult.AuthenticationResult.IdToken),
                    new Claim(CognitoMFASetupAccessToken, challengeResult.AuthenticationResult.AccessToken),
                    new Claim(CognitoMFASetupRefreshToken, challengeResult.AuthenticationResult.RefreshToken),
                    new Claim(CognitoMFASetupExpiresIn, challengeResult.AuthenticationResult.ExpiresIn.ToString()),
                    new Claim(CognitoMFASetupSecret, associateResponse.SecretCode),
                }, IdentityConstants.ApplicationScheme));
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<PasswordChangeInfo> RetrievePasswordChangeInfoAsync()
        {
            var result = await Context.AuthenticateAsync(IdentityConstants.TwoFactorUserIdScheme).ConfigureAwait(false);
            if (result?.Principal != null)
            {
                ...
                {
                    UserId = result.Principal.FindFirstValue(ClaimTypes.Name),
                    CognitoPasswordChangeWorkflowId = result.Principal.FindFirstValue(CognitoPasswordChangeWorkflowKey)
                };
            }
            return null;
        }
        ...
        /// <summary>
        /// Utility class to model information related to the ongoing password change workflow.
        /// </summary>
        internal class PasswordChangeInfo
        {
            public string UserId { get; set; }
            public string CognitoPasswordChangeWorkflowId { get; set; }
        }
