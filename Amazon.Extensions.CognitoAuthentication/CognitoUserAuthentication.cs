...
// ********** BEGIN MAATWERK **********
        public async Task<AuthFlowResponse> RespondToMfaSetupAsync(RespondToMfaSetupRequest mfaSetupRequest, CancellationToken cancellationToken)
        {
            if (mfaSetupRequest == null)
                throw new ArgumentNullException(nameof(mfaSetupRequest));
            ...
            RespondToAuthChallengeRequest challengeRequest = new RespondToAuthChallengeRequest
            {
                ...
                Session = mfaSetupRequest.SessionID,
                ClientId = ClientID,
                ChallengeName = ChallengeNameType.MFA_SETUP
            };

            return await RespondToAuthChallengeAsync(challengeRequest, cancellationToken);
        }

        public async Task<AuthFlowResponse> RespondToAuthChallengeAsync(RespondToAuthChallengeRequest challengeRequest, CancellationToken cancellationToken)
        {
            if (!string.IsNullOrEmpty(SecretHash))
                challengeRequest.ChallengeResponses.Add(CognitoConstants.ChlgParamSecretHash, SecretHash);
